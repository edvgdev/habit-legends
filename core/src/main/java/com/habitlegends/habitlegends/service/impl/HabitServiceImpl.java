...
public class HabitServiceImpl implements HabitService {
    ...
    public HabitDetails createHabit(HabitAndStatRewardsDetails habitAndStatRewardsDetails) {
        ...
        List<HabitStatRewardDTO> habitStatRewardDTOs = habitAndStatRewardsDetails.getStatRewards().stream()
                .map(statReward -> {
                    HabitStatRewardDTO habitStatRewardDTO = new HabitStatRewardDTO(null, savedHabit.getId(),
    ...
    public HabitDetails updateHabit(Long id, HabitAndStatRewardsDetails habitAndStatRewardsDetails) {
        ...
        Map<Integer, HabitStatRewardDTO> existingStatRewardsMap = existingStatRewards.stream()
                .collect(Collectors.toMap(HabitStatRewardDTO::getStatId, statRewardDTO -> statRewardDTO));
        ...
        List<HabitStatRewardDTO> fromPayload = habitAndStatRewardsDetails.getStatRewards().stream()
                .map(statRewardDetails -> {
                    ...
                    HabitStatRewardDTO existingReward = existingStatRewardsMap.get(statRewardDetails.getStatId());
                    if (existingReward != null) {
                        ...
                        return new HabitStatRewardDTO(
                                null, updatedHabit.getId(), statRewardDetails.getStatId(),
        ...
        existingStatRewards.stream()
                .filter(existingStatRewardDTO -> !incomingStatIds.contains(existingStatRewardDTO.getStatId()))
                .forEach(statReward -> habitStatRewardService.deleteHabitStatReward(statReward.getId()));

        String categoryName = updatedHabit.getCategory().getName();

        // Combine the updated stat rewards into the response DTO
        HabitDetails habitDetails = new HabitDetails(modelMapper.map(updatedHabit, HabitDTO.class), categoryName,
                statRewardsToSave);
    ...
    @Override
    public HabitDetails getHabitById(Long id) {
        Habit habit = habitRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Habit not found"));
